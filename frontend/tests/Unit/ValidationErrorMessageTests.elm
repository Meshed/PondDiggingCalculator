    ...
    describe "Validation Error Message Display"
        [ describe "hasValidationErrors State Management"
            [ test "starts with no validation errors" <|
                \_ ->
                    let
                        model = createModelWithValidData
                    in
            ...
            , test "remains false after successful calculation simulation" <|
                \_ ->
                    let
                        initialModel = createModelWithValidData
                        -- Simulate successful calculation
                        updatedModel = { initialModel | hasValidationErrors = False, calculationResult = Just createValidCalculationResult }
                    in
            ...
            , test "sets to true when form data parsing fails" <|
                \_ ->
                    let
                        modelWithBadData = createModelWithInvalidData
                        -- Simulate failed calculation due to invalid data
                        updatedModel = { modelWithBadData | hasValidationErrors = True }
                    in
            ...
            , test "resets to false when form is cleared" <|
                \_ ->
                    let
                        modelWithErrors = { createModelWithValidData | hasValidationErrors = True }
                        -- Simulate form clear operation
                        updatedModel = { modelWithErrors | hasValidationErrors = False, formData = Just createValidFormData, calculationResult = Nothing, lastValidResult = Nothing }
                    in
                    Expect.equal False updatedModel.hasValidationErrors
            ...
            , test "transitions from true to false when invalid input becomes valid" <|
                \_ ->
                    let
                        ...
                        modelWithErrors = { createModelWithInvalidData | hasValidationErrors = True }
                        -- Update with valid data and simulate successful calculation
                        validFormData = createValidFormData
                        updatedModel = { modelWithErrors | formData = Just validFormData, hasValidationErrors = False, calculationResult = Just createValidCalculationResult }
                    in
                    Expect.equal False updatedModel.hasValidationErrors
            ...
            , test "transitions from false to true when valid input becomes invalid" <|
                \_ ->
                    let
                        ...
                        validModel = createModelWithValidData
                        -- Update with invalid data and simulate failed calculation
                        invalidFormData = createInvalidFormData
                        updatedModel = { validModel | formData = Just invalidFormData, hasValidationErrors = True }
                    in
                    Expect.equal True updatedModel.hasValidationErrors
            ]
        ...
        , describe "isStale Logic in View"
            [ test "isStale is false when hasValidationErrors is false" <|
                \_ ->
                    let
                        model = { createModelWithValidData | hasValidationErrors = False }
            ...
            , test "isStale is true when hasValidationErrors is true" <|
                \_ ->
                    let
                        model = { createModelWithValidData | hasValidationErrors = True }
                        -- In the actual view logic: isStale = model.hasValidationErrors
                        isStale = model.hasValidationErrors
                    in
            ...
            , test "isStale correctly reflects state after successful calculation" <|
                \_ ->
                    let
                        initialModel = createModelWithValidData
                        modelAfterCalc = { initialModel | hasValidationErrors = False, calculationResult = Just createValidCalculationResult }
                        isStale = modelAfterCalc.hasValidationErrors
                    in
                    Expect.equal False isStale
            ...
            , test "isStale correctly reflects state after failed calculation" <|
                \_ ->
                    let
                        initialModel = createModelWithInvalidData
                        modelAfterCalc = { initialModel | hasValidationErrors = True }
                        isStale = modelAfterCalc.hasValidationErrors
                    in
                    Expect.equal True isStale
            ]
        ...
        , describe "Validation Message Display Scenarios"
            [ test "message should NOT show with fresh valid inputs" <|
                \_ ->
                    let
                        model = createModelWithValidData
                        updatedModel = { model | hasValidationErrors = False, calculationResult = Just createValidCalculationResult }
                        shouldShowMessage = updatedModel.hasValidationErrors
                    in
            ...
            , test "message SHOULD show with invalid inputs but valid last result" <|
                \_ ->
                    let
                        ...
                        validResult = createValidCalculationResult
                        model = 
                        ...
                        shouldShowMessage = model.hasValidationErrors
                    in
            ...
            , test "message should NOT show after correcting invalid inputs" <|
                \_ ->
                    let
                        ...
                        -- Fix the inputs
                        validFormData = createValidFormData
                        correctedModel = { modelWithErrors | formData = Just validFormData, hasValidationErrors = False }
                        shouldShowMessage = correctedModel.hasValidationErrors
                    in
                    Expect.equal False shouldShowMessage
            ...
            , test "message should NOT show after form clear" <|
                \_ ->
                    let
                        modelWithErrors = { createModelWithValidData | hasValidationErrors = True }
                        updatedModel = { modelWithErrors | hasValidationErrors = False, calculationResult = Nothing, lastValidResult = Nothing }
                        shouldShowMessage = updatedModel.hasValidationErrors
                    in
                    Expect.equal False shouldShowMessage
            ]
        ...
        , describe "Regression Prevention"
            [ test "hasValidationErrors never gets stuck at true with valid inputs" <|
                \_ ->
                    let
                        ...
                        model1 = createModelWithValidData
                        model2 = { model1 | hasValidationErrors = False, calculationResult = Just createValidCalculationResult, lastValidResult = Just createValidCalculationResult }
                        model3 = { model2 | hasValidationErrors = False }
                        model4 = { model3 | hasValidationErrors = False }
                    in
                    Expect.all
            ...
            , test "validation state correctly persists across device type changes" <|
                \_ ->
                    let
                        mobileModel = { createModelWithValidData | deviceType = Mobile }
                        mobileAfterCalc = { mobileModel | hasValidationErrors = False, calculationResult = Just createValidCalculationResult }
            ...
            , test "lastValidResult and calculationResult equality doesn't affect hasValidationErrors" <|
                \_ ->
                    let
                        result = createValidCalculationResult
                        model = createModelWithValidData
                        modelAfterCalc = { model | calculationResult = Just result, lastValidResult = Just result, hasValidationErrors = False }
                        -- After successful calculation, lastValidResult == calculationResult
                        resultsAreEqual = modelAfterCalc.lastValidResult == modelAfterCalc.calculationResult
                        -- But hasValidationErrors should still be false
                        hasErrors = modelAfterCalc.hasValidationErrors
                    in
                    Expect.all
