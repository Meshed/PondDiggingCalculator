    ...
    describe "Validation State Integration Tests"
        [ describe "Cross-Device Validation State"
            [ test "validation errors persist when switching from Mobile to Desktop" <|
                \_ ->
                    let
                        mobileModel = createModelWithInvalidInputs Mobile
                        mobileAfterCalc = { mobileModel | hasValidationErrors = True }
                        desktopModel = { mobileAfterCalc | deviceType = Desktop }
                    in
                    Expect.all
            ...
            , test "valid state persists when switching from Desktop to Tablet" <|
                \_ ->
                    let
                        desktopModel = createModelWithValidInputs Desktop
                        desktopAfterCalc = { desktopModel | hasValidationErrors = False, calculationResult = Just createValidCalculationResult }
                        tabletModel = { desktopAfterCalc | deviceType = Tablet }
                    in
                    Expect.all
            ...
            , test "validation state independent of device type changes" <|
                \_ ->
                    let
                        model = createModelWithValidInputs Mobile
                        step1 = { model | hasValidationErrors = False, calculationResult = Just createValidCalculationResult }
                        step2 = { step1 | deviceType = Tablet }
                        step3 = { step2 | deviceType = Desktop }
                        step4 = { step3 | deviceType = Mobile }
                    in
                    Expect.all
                        [ \m -> Expect.equal False m.hasValidationErrors
                        , \m -> Expect.notEqual Nothing m.calculationResult
        ...
        , describe "Form Input State Scenarios"
            [ test "invalid then valid excavator capacity simulation" <|
                \_ ->
                    let
                        model = createModelWithValidInputs Desktop
                        -- Simulate invalid input
                        step1 = { model | hasValidationErrors = True }
                        -- Simulate correction to valid input
                        step2 = { step1 | hasValidationErrors = False, calculationResult = Just createValidCalculationResult }
                    in
                    Expect.all
                        [ \m -> Expect.equal False m.hasValidationErrors
                        , \m -> Expect.notEqual Nothing m.calculationResult
            ...
            , test "invalid truck data then form clear simulation" <|
                \_ ->
                    let
                        ...
                        step1 = { model | hasValidationErrors = True }
                        -- Simulate form clear
                        step2 = { step1 | hasValidationErrors = False, calculationResult = Nothing, lastValidResult = Nothing }
                    in
                    Expect.all
                        [ \m -> Expect.equal False m.hasValidationErrors
            ...
            , test "rapid sequential input changes simulation" <|
                \_ ->
                    let
                        ...
                        step1 = { model | hasValidationErrors = True }    -- invalid
                        step2 = { step1 | hasValidationErrors = False }   -- valid
                        step3 = { step2 | hasValidationErrors = True }    -- invalid
                        step4 = { step3 | hasValidationErrors = False, calculationResult = Just createValidCalculationResult }  -- valid
                    in
                    Expect.all
                        [ \m -> Expect.equal False m.hasValidationErrors
                        , \m -> Expect.notEqual Nothing m.calculationResult
            ...
            , test "mixed valid and invalid field updates simulation" <|
                \_ ->
                    let
                        ...
                        step1 = { model | hasValidationErrors = False }  -- valid update
                        step2 = { step1 | hasValidationErrors = True }   -- invalid update
                        step3 = { step2 | hasValidationErrors = True }   -- still invalid
                    in
                    Expect.all
                        [ \m -> Expect.equal True m.hasValidationErrors
        ...
        , describe "Calculation Pipeline Simulation"
            [ test "successful calculation clears validation errors" <|
                \_ ->
                    let
                        ...
                        baseModel = createModelWithValidInputs Desktop
                        modelWithErrors = { baseModel | hasValidationErrors = True }
                        -- Simulate successful calculation
                        result = { modelWithErrors 
            ...
            , test "failed calculation sets validation errors" <|
                \_ ->
                    let
                        modelWithInvalidData = createModelWithInvalidInputs Desktop
                        -- Simulate failed calculation
                        result = { modelWithInvalidData 
            ...
            , test "validation errors preserved during calculation in progress" <|
                \_ ->
                    let
                        model = createModelWithInvalidInputs Desktop
                        modelInProgress = { model | calculationInProgress = True, hasValidationErrors = True }
                        -- Validation errors should persist
                        result = { modelInProgress | hasValidationErrors = True }
                    in
            ...
            , test "calculation result consistency with validation state" <|
                \_ ->
                    let
                        validModel = createModelWithValidInputs Desktop
                        validResult = { validModel | hasValidationErrors = False, calculationResult = Just createValidCalculationResult }
                        
                        invalidModel = createModelWithInvalidInputs Desktop
                        invalidResult = { invalidModel | hasValidationErrors = True, calculationResult = Nothing }
                    in
                    Expect.all
        ...
        , describe "State Persistence Edge Cases"
            [ test "validation state with performance tracking" <|
                \_ ->
                    let
                        model = createModelWithValidInputs Desktop
                        afterCalc = { model | hasValidationErrors = False, calculationResult = Just createValidCalculationResult }
                        -- Performance tracking shouldn't affect validation state
                        afterPerfTracking = afterCalc
                    in
                    Expect.all
                        [ \m -> Expect.equal False m.hasValidationErrors
                        , \m -> Expect.notEqual Nothing m.calculationResult
            ...
            , test "validation state with device type changes" <|
                \_ ->
                    let
                        model = createModelWithInvalidInputs Desktop
                        afterCalc = { model | hasValidationErrors = True }
                        -- Device type change shouldn't affect validation state
                        afterResize = { afterCalc | deviceType = Tablet }
                    in
                    Expect.all
        ...
        , describe "Regression Prevention Scenarios"
            [ test "multiple calculations with same valid data don't create false errors" <|
                \_ ->
                    let
                        model = createModelWithValidInputs Desktop
                        result1 = { model | hasValidationErrors = False, calculationResult = Just createValidCalculationResult, lastValidResult = Just createValidCalculationResult }
                        result2 = { result1 | hasValidationErrors = False }
                        result3 = { result2 | hasValidationErrors = False }
                        result4 = { result3 | hasValidationErrors = False }
                    in
                    Expect.all
                        [ \m -> Expect.equal False m.hasValidationErrors
                        , \m -> Expect.notEqual Nothing m.calculationResult
            ...
            , test "alternating valid/invalid inputs maintain correct state" <|
                \_ ->
                    let
                        ...
                        step1 = { model | hasValidationErrors = False }    -- valid
                        step2 = { step1 | hasValidationErrors = True }     -- invalid  
                        step3 = { step2 | hasValidationErrors = False }    -- valid
                        step4 = { step3 | hasValidationErrors = True }     -- invalid
                        step5 = { step4 | hasValidationErrors = False, calculationResult = Just createValidCalculationResult }  -- valid
                    in
                    Expect.all
                        [ \m -> Expect.equal False m.hasValidationErrors
                        , \m -> Expect.notEqual Nothing m.calculationResult
            ...
            , test "validation state isolation between form fields" <|
                \_ ->
                    let
                        ...
                        withBadExcavator = { model | hasValidationErrors = True }
                        fixedExcavator = { withBadExcavator | hasValidationErrors = False }
                        withBadPond = { fixedExcavator | hasValidationErrors = True }
                        allFixed = { withBadPond | hasValidationErrors = False }
                    in
