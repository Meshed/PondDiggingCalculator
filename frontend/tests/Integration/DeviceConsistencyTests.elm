        ...
        , test "documents_state_isolation_problem" <|
            \_ ->
                let
                    ...
                    ( initialMobile, _ ) = Mobile.init
                    ( mobileWithInput, _ ) = Mobile.update (Mobile.ExcavatorCapacityChanged "5.0") initialMobile
                    
                    -- This data exists ONLY in mobile state
                    mobileExcavatorValue = mobileWithInput.excavatorCapacity
                    
                    -- Desktop formData would be completely separate
                    desktopFormData = Nothing  -- No connection to mobile input!
                in
        ...
        , test "documents_calculation_isolation_problem" <|
            \_ ->
                let
                    ...
                    ( initialMobile, _ ) = Mobile.init
                    mobileWithConfig = { initialMobile | config = Just Config.fallbackConfig }
                    ( mobileWithResult, _ ) = Mobile.update (Mobile.ExcavatorCapacityChanged "3.0") mobileWithConfig
                    
                    -- Desktop has separate calculation result
                    desktopResult = Nothing  -- Completely separate!
                in
                case mobileWithResult.result of
                    ...
                    Nothing ->
                        Expect.pass  -- Both empty, but still demonstrates separation
        ]