...
import Utils.Calculations exposing (Bottleneck(..), CalculationResult, ConfidenceLevel(..), calculateTimeline)
            ...
            [ test "should_display_enhanced_input_fields_with_units_and_help_text" <|
                \_ ->
                    let
                        ...
                        inputFieldHtml =
                            ProjectForm.inputFieldWithUnit Desktop
                                { label = "Test Field"
                                , unit = "cubic yards"
            ...
            , test "should_render_equipment_cards_with_visual_icons" <|
                \_ ->
                    let
                        ...
                        cardHtml =
                            EquipmentCard.view Desktop config (\_ -> ())

                        queryResult =
                            Query.fromHtml cardHtml
                    in
                    queryResult
            ...
            , test "should_display_enhanced_results_with_additional_metrics" <|
                \_ ->
                    let
                        ...
                        resultsHtml =
                            ResultsPanel.view Desktop testResult False

                        queryResult =
                            Query.fromHtml resultsHtml
                    in
                    queryResult
            ...
            , test "should_adapt_layout_for_different_device_types" <|
                \_ ->
                    let
                        ...
                        tabletHtml =
                            ResultsPanel.view Tablet testResult False

                        mobileHtml =
                            ResultsPanel.view Mobile testResult False

                        desktopQuery =
                            Query.fromHtml desktopHtml

                        tabletQuery =
                            Query.fromHtml tabletHtml

                        mobileQuery =
                            Query.fromHtml mobileHtml
                    in
                    Expect.all
            ...
            [ test "should_produce_valid_calculation_with_form_defaults" <|
                \_ ->
                    let
                        ...
                        maybeWork =
                            String.toFloat formData.workHoursPerDay
                    in
                    case maybeExc of
                                            ...
                                            case maybeWork of
                                                Just workHours ->
                                                    let
                                                        ...
                                                        result =
                                                            calculateTimeline excCap excCycle truckCap truckRound pondVolume workHours
                                                    in
                                                    case result of
                                                        Ok calculation ->
            ...
            , test "should_validate_default_values_successfully" <|
                \_ ->
                    let
                        ...
                        validationResult =
                            Validation.validateAllInputs fallbackConfig.validation inputs
                    in
            ...
            [ test "should_update_excavator_capacity_field" <|
                \_ ->
                    let
                        ...
                        updated =
                            ProjectForm.updateFormData (ProjectForm.UpdateExcavatorCapacity "5.0") form
                    in
            ...
            , test "should_update_cycle_time_field" <|
                \_ ->
                    let
                        ...
                        updated =
                            ProjectForm.updateFormData (ProjectForm.UpdateExcavatorCycleTime "3.0") form
                    in
            ...
            , test "should_update_truck_capacity_field" <|
                \_ ->
                    let
                        ...
                        updated =
                            ProjectForm.updateFormData (ProjectForm.UpdateTruckCapacity "15.0") form
                    in
            ...
            , test "should_update_work_hours_field" <|
                \_ ->
                    let
                        ...
                        updated =
                            ProjectForm.updateFormData (ProjectForm.UpdateWorkHours "10.0") form
                    in
            ...
            , test "should_update_pond_dimensions" <|
                \_ ->
                    let
                        ...
                        updated1 =
                            ProjectForm.updateFormData (ProjectForm.UpdatePondLength "100.0") form

                        updated2 =
                            ProjectForm.updateFormData (ProjectForm.UpdatePondWidth "50.0") updated1

                        updated3 =
                            ProjectForm.updateFormData (ProjectForm.UpdatePondDepth "10.0") updated2
                    in
                    Expect.all
